...
struct Config {
    ...
    // Generated by the VS Code client. The TUI does not use these values, but
    // must preserve them when it updates the shared config file.
    #[serde(skip_serializing_if = "Option::is_none")]
...
async fn fetch_history(url: &str, key: &str) -> Result<Vec<HistoryRow>, String> {
    let endpoint = format!(
        "{}/rest/v1/messages?select=id,username,text,created_at,edited_at&order=id.desc&limit={HISTORY_LIMIT}",
        ...
        tokio::select! {
            ...
            ev = async {
                ...
                match ev {
                    ...
                    Some(RtEvent::Chat { id, username, text, timestamp_ms }) => {
